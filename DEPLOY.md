...
| `cover.html` | **ไฟล์ใหม่** — หน้าปกนำเสนอ |
...
## 2. หลังอัปเสร็จ
กด `Ctrl + Shift + R` (ล้าง cache) — sw.js ขยับเป็น **wnb-v9** แล้ว
ระบบจะล้าง cache เก่าให้ผู้ใช้ทุกคนอัตโนมัติ

## 3. สิ่งที่เปลี่ยนใน sw.js
- `VERSION` : wnb-v6 → **wnb-v9**
...
## 8. หน้าปกนำเสนอ (cover.html)

ใช้เปิดก่อนเข้าแดชบอร์ดในการประชุม หรือฉายค้างไว้บนจอ

**ใส่ภาพพื้นหลังของตัวเอง**
วางไฟล์ชื่อ `cover-bg.jpg` ไว้โฟลเดอร์เดียวกับ `cover.html`
หน้าปกจะโหลดขึ้นมาเองโดยอัตโนมัติ (ค่อย ๆ จางเข้า)
ถ้าไม่มีไฟล์นี้ ระบบจะใช้พื้นหลังไล่เฉดสีกรมท่า + ลายเส้นขอบเขตจังหวัดแทน — ไม่พัง

- แนะนำภาพแนวนอน อย่างน้อย 1920×1080 px
- ระบบจะคลุมด้วยชั้นสีเข้มโปร่งแสงให้อัตโนมัติ ตัวหนังสือจึงอ่านออกเสมอ

**ตั้งให้ปกเป็นหน้าแรกของเว็บ (ถ้าต้องการ)**
เปลี่ยนชื่อไฟล์: `index.html` → `dashboard.html` และ `cover.html` → `index.html`
จากนั้นแก้ลิงก์ปุ่ม "เข้าสู่แดชบอร์ด" ในปกให้ชี้ไป `dashboard.html`
พร้อมแก้เมนูด้านข้างของทุกหน้าที่ชี้ `index.html` ให้เป็น `dashboard.html`
(ถ้าไม่อยากแก้หลายจุด แนะนำให้เปิดปกที่ `cover.html` ตามเดิม)
